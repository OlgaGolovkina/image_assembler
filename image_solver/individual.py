...
class Individual:
    """Class representing possible solution to image puzzle.

    Individual object is one of the solutions to the problem
    (possible arrangement of the image's pieces).
    It is created by random shuffling initial image.

    :param pieces:  Array of pieces representing initial image.
    ...
    def __init__(self, pieces, rows, columns, shuffle=True):
        ...
        if shuffle:
            ...
            np.random.shuffle(self._pieces)
            self.pieces = [self.pieces[0]] + self._pieces
